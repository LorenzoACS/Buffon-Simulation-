...
public class Tarp {
    ...
    public int numberOfCrossings() {
        for (int i = 0; i < needles.size(); i++) {
            if (needles.get(i).leftTip() == 0.0) {
                counter++;
            } else if (needles.get(i).rightTip() == 0.0) {
                counter++;
            } else if (needles.get(i).leftTip() == this.t) {
                counter++;
            } else if (needles.get(i).rightTip() == this.t) {
                counter++;
            }   
        }
        return counter;
    ...
    public void addNeedle() {
        needles.add(new Needle(this.length, this));
    }
}
